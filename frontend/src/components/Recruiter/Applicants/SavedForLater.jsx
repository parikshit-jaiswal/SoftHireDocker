    ...
    return (
        <div className="bg-white rounded-lg border border-gray-300 w-full hover:shadow-md transition-shadow duration-200  p-3 sm:p-4">
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                {/* Profile Section */}
                <div onClick={() => navigate(`/recruiter/applicants/profile/${applicant.userId}`)} className="flex items-center space-x-3 md:space-x-4 flex-1 min-w-0 cursor-pointer">
                ...
                <div className="flex flex-col sm:flex-row gap-2 sm:gap-3 md:flex-shrink-0 w-full md:w-auto">
                    <div className="flex flex-col gap-2 w-full sm:w-auto sm:items-start">
                        <p className='text-red-500 text-sm sm:text-base text-left w-full'>Saved on {applicant?.statusUpdatedAt ? new Date(applicant.statusUpdatedAt).toLocaleDateString() : 'N/A'}</p>
